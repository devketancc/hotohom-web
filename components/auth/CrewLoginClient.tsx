...
export function CrewLoginClient() {
  const router = useRouter();
  const { user, isAuthenticated, sendOtp, resendOtp, verifyCrewOtp } = useAuth();
  ...
  const alreadyCrewSession = Boolean(isAuthenticated && user && isCrewRole(user.role));

  useEffect(() => {
    if (!alreadyCrewSession) return;
    router.replace('/crew/roster');
  }, [alreadyCrewSession, router]);
  ...
  }, [phone, otp, verifyCrewOtp, router]);
  ...
  if (alreadyCrewSession) {
    return (
      <div className="dark flex min-h-screen flex-col items-center justify-center bg-stitch-background px-4 py-12 text-stitch-on-background">
        <MotohomLogo className="mb-6 h-9 w-auto max-w-[min(100%,16rem)]" blendOnDark />
        <p className="font-body text-sm text-stitch-on-surface-variant">Taking you to your roster…</p>
      </div>
    );
  }
