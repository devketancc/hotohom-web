...
export const BookingControl = () => {
  ...
  return (
    <div className="luxury-glass luxury-glass-focus booking-panel-reflection rounded-3xl p-2 md:p-2.5 flex flex-col gap-2 md:h-[96px] md:flex-row md:items-stretch md:gap-0 transition-all duration-500 ease-out md:hover:-translate-y-0.5 md:hover:shadow-[0_26px_72px_-30px_rgba(0,0,0,0.75)]">
      <div className="flex-1 flex min-h-[74px] items-stretch rounded-2xl">
        <HubSelector />
      </div>
      <div aria-hidden className="hidden md:block my-4 w-px bg-white/[0.08]" />
      <div className="flex-1 flex min-h-[74px] items-stretch rounded-2xl">
        <DatePicker />
      </div>
      <div aria-hidden className="hidden md:block my-4 w-px bg-white/[0.08]" />

      <div ref={guestsRef} className="relative flex-1 min-h-[74px]">
        <button
          type="button"
        ...
        >
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <span className="text-[15px] font-medium text-stitch-on-background truncate">
              {passengers} {passengers > 1 ? 'Guests' : 'Guest'}{pets > 0 ? `, ${pets} Pet${pets > 1 ? 's' : ''}` : ''}
            </span>
          </div>
        </button>
          ...
          <div className="absolute bottom-[calc(100%+12px)] left-0 z-[100] w-[260px] rounded-2xl border border-white/10 bg-stitch-surface/90 p-4 backdrop-blur-2xl shadow-[0_28px_60px_-20px_rgba(0,0,0,0.65)] animate-in fade-in slide-in-from-bottom-3 duration-200">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] uppercase tracking-[0.18em] text-stitch-on-surface-variant/80">Guests</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-[11px] uppercase tracking-[0.18em] text-stitch-on-surface-variant/80">Pets</span>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
