...
function formatQueryError(err: unknown): string {
  ...
  if (code === 'CREW_API_UNAVAILABLE') return CREW_API_UNAVAILABLE_MESSAGE;
  ...
  if (msg.includes('404') || msg.includes('not found')) return CREW_API_UNAVAILABLE_MESSAGE;
  return err.message;
}

function groupBookingsByLocalDay(bookings: AdminRosterBooking[]): { dayKey: string; items: AdminRosterBooking[] }[] {
  const sorted = [...bookings].sort((a, b) => parseISO(a.start).getTime() - parseISO(b.start).getTime());
  ...
  return [...map.keys()].sort().map((dayKey) => ({ dayKey, items: map.get(dayKey)! }));
}
...
export function CrewRosterPanel() {
  ...
  const { data, dataSource, isPending, error, validation } = useCrewCalendarRoster(filters);
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        {dataSource === 'calendar' ? (
          <p className="text-[11px] text-muted-foreground">Showing assignments from your calendar schedule.</p>
        ) : (
          <span />
        )}
        <button
      ...
      {error ? (
        <div className="rounded-md border border-red-500/20 bg-red-900/10 px-3 py-2 text-xs text-red-300" role="alert">
      ...
      {!isPending && !error && validation.ok && (data?.length ?? 0) === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-md border border-dashed border-border/50 bg-muted/10 px-4 py-10 text-center">
          ...
          <p className="mt-1 max-w-md text-xs text-muted-foreground">Try widening the date range.</p>
        </div>
      ) : null}
